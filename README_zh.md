...
大模型长文本交互中，很多开发者常有误区：*“既然 Antigravity 超限时会自动插入 Checkpoint 摘要截断，为什么还需要手动提炼？”*

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                  🚨 为什么底层自动截断不够？主动提炼才是专业工程解法            │
├─────────────────────────┬────────────────────────────────────────────────────┤
│ 维度                    │ 🤖 Antigravity 底层被动截断                        │ ⚡ Cockpit 主动智能快照提炼 (推荐)                 │
├─────────────────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────────────┤
│ **触发时机**            │ **临界死线**（额度饱和到 95%~100% 爆满时才被迫触发） │ **黄金节点**（完成一个里程碑、注意力衰减到 70% 时）  │
├─────────────────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────────────┤
│ **提炼质量**            │ **机械截断**（粗暴裁剪前文，容易丢失核心架构细节）   │ **高密度结构化提炼**（剥离数万行终端调试噪音，沉淀核心事实）│
├─────────────────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────────────┤
│ **模型注意力 (Attention)**| **持续衰减**（长期背负数十兆工具输出，产生“失忆/幻觉”）│ **即刻重置**（瞬间卸下历史包袱，模型推理重回 100% 敏捷） │
├─────────────────────────┼────────────────────────────────────────────────────┼────────────────────────────────────────────────────┤
│ **资产沉淀**            │ **无落盘**（会话一旦损坏或重开，前文记忆彻底蒸发）   │ **物理归档**（生成带时间戳快照并同步 `memory.md`，工程可追溯）│
└─────────────────────────┴────────────────────────────────────────────────────┴────────────────────────────────────────────────────┘
```

1. **摆脱注意力泥潭（Attention Decay）**：在长达几十轮的复杂开发中，过往海量的终端工具执行日志会严重污染注意力。在 70% 时主动提炼，能彻底清空噪音，让模型恢复 100% 敏捷思考；
2. **拒绝灾难性遗忘**：底层被动截断容易误删顶层架构规则；主动提炼由开发者把控节奏，确保核心交付与约束 100% 保留；
3. **沉淀永续工程资产**：生成的 `docs/snapshots/snapshot_*.md` 与 `memory.md` 指针，让后续新会话或其他 Agent 能够直接秒级无缝接力。
